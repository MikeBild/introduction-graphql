...
const database = new PouchDB(`database.sqlite`, {adapter: 'websql', auto_compaction: true});
...
function find (database, ids) {
    ...
    .then(data => data.reduce((s,n) => {s = s.concat(n); return s;}, []))
...
function upsert (database, name, entity) {
    ...
    return database.put(doc)
        .then(data => { delete data.ok; return data; })
