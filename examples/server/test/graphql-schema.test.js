...
describe('Database integration - GraphQL query and mutation schema', () => {
  describe('for author entity', () => {
    ...
    after(() => {
      return Promise.all([authors.removeAll(), posts.removeAll()]);
    });
    ...
    it('Query fetch_author with id: John Doe should return the author', () => {
      return graphql
        .graphql(
          ...
          { root: {} },
          { user: { id: 'demo@domain.local' }, loaders: loaders() },
          { id: 'John Doe' },
        )
        .then(actual => {
          const expected = {
    ...
    it('Mutation insert_author should return created author', () => {
      return graphql
        .graphql(
          schema,
          `mutation insert_author($input: AuthorInput!) {
          ...
          { root: {} },
          { user: { id: 'demo@domain.local' }, loaders: loaders() },
          { input: { id: 'Insert User', name: 'Insert User' } },
        )
        .then(actual => {
          assert.deepEqual(actual.data.upsertAuthor, {
            id: 'Insert User',
            name: 'Insert User',
          });
        });
    ...
    it('Mutation update_author should return changed author', () => {
      return graphql
        ...
        .then(actual =>
          graphql.graphql(
            schema,
            `mutation update_author($input: AuthorInput!) {
                upsertAuthor(input: $input) {
                    id
                    name
                }
            ...
            {
              input: {
                id: actual.data.author.id,
                rev: actual.data.author.rev,
                name: 'Changed User',
              },
            },
          ),
    ...
    it('Mutation remove_author should return remove author ID', () => {
      return graphql
        ...
        .then(actual =>
          graphql.graphql(
            schema,
            `mutation remove_author($input: AuthorInput!) {
                removeAuthor(input: $input)
            }`,
            { root: {} },
            { user: { id: 'demo@domain.local' }, loaders: loaders() },
            {
              input: { id: actual.data.author.id, rev: actual.data.author.rev },
            },
          ),
        )
        .then(actual => {
  ...
  describe('for post entity', () => {
    ...
    it('Mutation insert_post should return created post', () => {
      return graphql
        .graphql(
          schema,
          `mutation insert_post($input: PostInput!) {
      ...
      return graphql
        ...
        .then(actual =>
          graphql.graphql(
            ...
            {
              input: {
                id: actual.data.post.id,
                rev: actual.data.post.rev,
    ...
    it('Mutation remove_post should return changed author', () => {
      return graphql
        ...
        .then(actual =>
          graphql.graphql(
            schema,
            `mutation remove_post($input: PostInput!) {
                removePost(input: $input)
            }`,
            { root: {} },
            { user: { id: 'demo@domain.local' }, loaders: loaders() },
            { input: { id: actual.data.post.id, rev: actual.data.post.rev } },
          ),
        )
        .then(actual => {
