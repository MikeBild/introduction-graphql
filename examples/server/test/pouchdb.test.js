...
describe('PouchDB database queries should', () => {
  ...
  after(() => {
    return Promise.all([demo1Docs.removeAll(), demo2Docs.removeAll()]);
  });

  it('findAll demo1Docs yield 1 documents', () => {
    return demo1Docs.findAll().then(data => {
      assert.equal(data.length, 1);
    });
  });

  it('findAll demo1Docs yield 2 documents', () => {
    return demo2Docs.findAll().then(data => {
      assert.equal(data.length, 2);
    });
  });

  it('find by IDs yield 3 documents', () => {
    return database.find(['A', 'B', 'C']).then(data => {
      assert.equal(data.length, 3);
    });
  });

  it('find by 2 IDs yield 2 documents', () => {
    return database.find(['A', 'K']).then(data => {
      assert.equal(data.length, 2);
    });
  });

  it('get by ID yield a document', () => {
    return database.get('A').then(data => {
      assert.equal(data.id, 'A');
    });
  });

  it('get by ID yield a document', () => {
    return demo1Docs.get('A').then(data => {
      assert.equal(data.id, 'A');
    });
  });
});
